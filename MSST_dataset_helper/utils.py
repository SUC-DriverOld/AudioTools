...
def match_lengths(arr1, arr2):
    ...
    if len1 < max_len:
        padding = np.zeros((arr1.shape[0], max_len - len1, arr1.shape[2]))
        arr1 = np.concatenate((arr1, padding), axis=1)
    elif len2 < max_len:
        padding = np.zeros((arr2.shape[0], max_len - len2, arr2.shape[2]))
        arr2 = np.concatenate((arr2, padding), axis=1)
    
    return arr1, arr2

def sdr(references, estimates):
    references, estimates = match_lengths(references, estimates)
